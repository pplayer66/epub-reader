...
EPUBJS.Reader.prototype.addBookmark = function(cfi) {
		...
		if(present > -1 ) return;
		$.ajax({
				url : `/reader/addbookmark?bm=${cfi}`,
				type : 'GET',
				success : function(data){console.log('data', data)},
				error : function(error){console.log('err', err)}
		})
...
EPUBJS.reader.ControlsController = function(book) {
		...
		$slider.on("click", function () {
				if(reader.sidebarOpen) {
						...
						$slider.removeClass("icon-left");
				} else {
						reader.SidebarController.show();
						$slider.addClass("icon-left");
	...
	book.on('renderer:selected', function(range){
		if (range) {
			...
			if (text.length > 0){
				...
				$('#description-meta').attr('property', 'og:description');
				$('#description-meta').attr('content', text);
			}
		}
	});
